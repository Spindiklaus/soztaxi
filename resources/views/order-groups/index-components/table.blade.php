...
<!-- Обертка для прокрутки таблицы -->
    <table class="min-w-full divide-y divide-gray-200 bg-white">
        <thead class="bg-gray-50 text-gray-200 sticky top-0 z-10 shadow-lg">
            <tr>
                <th scope="col" class="px-6 py-1 text-left text-xs font-medium text-gray-500 tracking-wider">
                ...
                <th scope="col" class="px-6 py-1 text-center text-xs font-medium text-gray-500 tracking-wider">
                    Кол-во заказов в поездке со статусом "Принят"
                </th>
<!--                <th scope="col" class="px-6 py-1 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
            @forelse($orderGroups as $group)
                <tr>
                    ...
                    <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-500 text-center">{{ $group->orders_count }}</td>
                    <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-500 text-center">
                        @if ($group->accepted_orders_count!=0)
                            {{ $group->accepted_orders_count }}
                        @endif    
                    </td>
    <!--                <td class="px-6 py-2 whitespace-nowrap text-sm text-gray-500">{{-- $group->taxi_way --}}</td>
                    ...
                    <td class="px-6 py-2 whitespace-nowrap text-left text-sm font-medium">
                        ...
                        <!-- Кнопка Удалить -->
                        @if ($group->accepted_orders_count==$group->orders_count)
