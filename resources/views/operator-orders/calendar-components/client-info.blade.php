    ...
    <div class="bg-gray-50 px-4  rounded-t-lg">
        <button type="button"
            ...
            <h2 class="text-lg font-semibold text-gray-800">
                {{ $client->fio }}
                <span class="text-sm">
                    (Выбрана поездка: {{$latestOrder->visit_data->format('d.m.Y H:i')}}, маршрут: 
                    <span class="bg-green-100">{{ Str::limit($latestOrder->adres_otkuda,40)}} - {{ Str::limit($latestOrder->adres_kuda,40)}})</span>
                </span>
            </h2>
