<!-- operator-orders.index-components/table_soz.blade.php -->
...
<!--                    <th scope="col" class="px-1 py-2 text-left text-xs font-bold uppercase tracking-wider">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse ($orders as $order)
                    ...
                    <!-- убрали по требованию операторов -->
<!--                    <td class="px-6 py-4">
                    ...
                    <td class="px-1 py-0"> <!-- действия оператора -->
                        <div class="flex flex-nowrap gap-1">
                            ...
                            @if(!$order->deleted_at)
                                @if($status->id == 1)
                                    ...
                                    <form action="{{ route('social-taxi-orders.destroy', array_merge(['social_taxi_order' => $order], $urlParams)) }}" method="POST" class="inline" onsubmit="return confirm('Вы уверены, что надо удалить заказ?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="inline-flex items-center px-3 py-1 bg-red-100 text-red-800 rounded-md hover:bg-red-200 text-sm " title="Удалить">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            ...
                            @if(!$order->deleted_at && !$order->cancelled_at)
                                ...
                                @endphp
                                @if($statusId == 1)
                                    <a href="{{ route('social-taxi-orders.cancel.form', array_merge(['social_taxi_order' => $order], $urlParams)) }} " title="Отменить"
