<!-- operator-orders.index-components/table_la.blade.php -->
...
<!--                                <div class="text-sm font-medium text-red-600">
                                ...
                                <div class="text-sm text-red-500" title="заказ от {{ $order->pz_data->format('d.m.Y H:i') }}">
                        ...
                        <td class="px-6 py-0">
                            @if($order->visit_data)
                                ...
                                @if($order->visit_obratno)
                                    <div class="text-lg font-medium text-gray-600 mt-1"  title="{{ $order->adres_obratno }}">
                                        Обратно: <span class="text-lg">{{ $order->visit_obratno->format('H:i') }}</span>
                                    </div>
                                @endif                            
                            @endif
                        </td>
                            ...
                            <div class="flex  flex-nowrap gap-1">
                                ...
                                @if(!$order->deleted_at)
                                    @if($status->id == 1)
                                        ...
                                        @endphp
                                        <a href="{{ $editRoute }}" class="inline-flex items-center px-3 py-1 bg-yellow-100 text-yellow-800 rounded-md hover:bg-yellow-200 text-sm" title="Редактировать">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                            </svg>
                                        </a>
                                        <!-- Кнопка УДАЛИТЬ в actions.blade.php -->
                                        <form action="{{ route('social-taxi-orders.destroy', array_merge(['social_taxi_order' => $order], $urlParams)) }}" method="POST" class="inline" 
                                              onsubmit="return confirm('Вы уверены, что надо удалить заказ?')" title="Удалить заказ">
                                            @csrf
                                            @method('DELETE')
                                    ...
                                    @else
                                        ...
                                        <button type="button" class="inline-flex items-center px-3 py-1 bg-gray-300 text-gray-500 rounded-md text-sm cursor-not-allowed" 
                                            disabled title="Удаление возможно только для заказов со статусом 'Принят'">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                            </svg>
                                        </button>
                                ...
                                @else    
                                    <form action="{{ route('social-taxi-orders.restore', array_merge(['social_taxi_order' => $order], $urlParams)) }}" method="POST" class="inline">
                                        @csrf
                                        @method('PATCH')
                                            <button type="submit" class="inline-flex items-center px-3 py-1 bg-green-100 text-green-800 rounded-md hover:bg-green-200 text-sm " title="Восстановить">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <!-- Кнопка копирования -->
                                @if(!$order->client->rip_at)
                                    <a href="{{ route('social-taxi-orders.create.by-type', array_merge(['type' => $order->type_order, 'copy_from' => $order->id], $urlParams)) }}" title = "Копировать"
                                ...
                                @else
                                    <button
                                       class="inline-flex items-center px-3 py-1 bg-gray-300 text-gray-500 rounded-md hover:bg-green-200 text-sm cursor-not-allowed"
                                ...
                                @if(!$order->deleted_at && !$order->cancelled_at)
                                    ...
                                    @if($statusId == 1)
                                        <a href="{{ route('social-taxi-orders.cancel.form', array_merge(['social_taxi_order' => $order], $urlParams)) }}" 
                                            class="inline-flex items-center px-3 py-1 bg-red-100 text-red-800 rounded-md hover:bg-red-200 text-sm" title="Отменить">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
