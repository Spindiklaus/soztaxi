    ...
    <div class="space-y-4">
        ...
        <div>
            ...
            <select name="taxi_id" id="taxi_id" 
                ...
                <option value="{{ $taxi->id }}" {{ ($order->taxi_id == $taxi->id) ? 'selected' : '' }}>
                    {{ $taxi->name }} (#{{ $taxi->id }})
                </option>
                @endforeach
            </select>
    ...
    <button type="button" 
        ...
        <div>
            ...
            <textarea name="adres_obratno" id="adres_obratno" 
                      ...
                      class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 {{ ($order->zena_type == 2 || $order->zena_type == 3) && ($order->zena_type == '1') ? 'bg-gray-100 cursor-not-allowed' : '' }}">{{ $order->adres_obratno }}</textarea>
            @error('adres_obratno')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
