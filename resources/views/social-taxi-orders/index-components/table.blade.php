...
<!--                            @if($order->taxi_price - $order->taxi_vozm<>0)
                        ...
                        <td class="px-4 py-0"> <!-- действия оператора -->
                            <div class="flex flex-nowrap gap-1">
                                ...
                                @if(!$order->deleted_at)
                                    @if($status->id == 1)
                                        <a href="{{ route('social-taxi-orders.edit', array_merge(['social_taxi_order' => $order], $urlParams)) }}" 
                                           class="inline-flex items-center px-3 py-1 bg-yellow-100 text-yellow-800 rounded-md hover:bg-yellow-200 text-sm" title="Редактировать">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
