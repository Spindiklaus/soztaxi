<x-app-layout>
    <div class="bg-gray-100 py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @endif
            <h1 class="text-3xl font-bold text-gray-800 mb-4">Создать новую скидку</h1>

            <form action="{{ route('skidka_dops.store') }}" method="POST" class="space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" name="skidka" id="skidka" value="{{ old('skidka') }}" min="50" max="100" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('skidka')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="kol_p" id="kol_p" value="{{ old('kol_p') }}" min="10" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                    @error('kol_p')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="life" id="life" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        <option value="1" {{ old('life') == '1' ? 'selected' : '' }}>Да</option>
                        <option value="0" {{ old('life') == '0' ? 'selected' : '' }}>Нет</option>
                    </select>
                    @error('life')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                    @enderror
